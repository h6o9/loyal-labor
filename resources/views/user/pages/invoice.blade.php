...
@section('title')
    {{ __('Invoice') }} || {{ $setting->app_name }}
@endsection

@section('user-breadcrumb')
    @include('components::breadcrumb', ['title' => __('Invoice'), 'image' => 'invoice'])
@endsection
            ...
            <div class="row justify-content-between">
                ...
                <div class="col-xl-4 col-sm-5">
                    <div class="wsus__invoice_date">
                        <h5>{{ __('Invoice') }}#<span>{{ $order->order_id }}</span></h5>
                        <h5 class="date">{{ __('Date') }}<span>{{ formattedDate($order->created_at) }}</span></h5>
                        <h5 class="date">
                            {{ __('Payment') }}<span>{{ $order->paymentDetails->payment_status->getLabel() }}</span>
                        </h5>
                        <h5 class="date">{{ __('Status') }}<span>{{ $order->order_status->getLabel() }}</span>
                        </h5>
                    </div>
                </div>
            </div>
            ...
            <div class="row">
                ...
                <div class="col-xl-6">
                    <div class="wsus__invoice_final_total_right">
                        ...
                        <h6>{{ __('Gateway Charge') }}:<span>{{ currency($order->gateway_fee) }}</span></h6>
                        <h5>{{ __('Total') }}: <span
