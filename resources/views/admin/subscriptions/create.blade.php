                            ...
                            <form action="{{ route('admin.subscriptions.store') }}" method="POST">
                                ...
                                <div class="row">
                                    <div class="form-group col-12">
                                        ...
                                        <select name="plan_type" id="plan_type" class="form-control" required>
                                            ...
                                            @foreach($planTypes as $value => $label)
                                                <option value="{{ $value }}" @selected(old('plan_type', 'basic_plan') === $value)>{{ __($label) }}</option>
                                            @endforeach
                                        </select>
                                    ...
                                    <div class="form-group col-12 col-md-6">
                                        ...
                                        <input type="number" id="duration_months" class="form-control" name="duration_months" value="{{ old('duration_months', 1) }}" required min="1" placeholder="e.g., 7, 15, 30">
                                    ...
                                    <div class="form-group col-12 col-md-6">
                                        ...
                                        <select name="duration_unit" id="duration_unit" class="form-control" required>
                                            @foreach(\App\Models\Subscription::DURATION_UNITS as $value => $label)
                                                <option value="{{ $value }}" @selected(old('duration_unit', 'months') === $value)>{{ __($label) }}</option>
                                            @endforeach
                                        </select>
                                    </div>
