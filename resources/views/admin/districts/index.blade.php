...
$(document).ready(function() {
    initServerDataTable('#districtTable', "{{ route('admin.districts.index') }}", [
        ...
        drawCallback: function () {
            ...
            // (do NOT load bootstrap4-toggle CDN — conflicts with local bootstrap5-toggle)
        }
    });
});
...
function changeDistrictStatus(id) {
    ...
    $.ajax({
        ...
        error: function(xhr){
            // #region agent log
            fetch('http://127.0.0.1:7600/ingest/506040b6-b118-437d-ac00-d7a82a44f1c6',{method:'POST',headers:{'Content-Type':'application/json','X-Debug-Session-Id':'8fd336'},body:JSON.stringify({sessionId:'8fd336',runId:'pre-fix',hypothesisId:'E',location:'districts/index.blade.php:error',message:'district status ajax error',data:{id:id,status:xhr.status,responseText:(xhr.responseText||'').substring(0,300)},timestamp:Date.now()})}).catch(function(){});
            // #endregion
