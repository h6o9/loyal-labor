...
class SslCommerzNotification extends AbstractSslCommerz
{
    ...
    public function __construct()
    {
        ...
        $this->sslcommerz_config = cache()->get('payment_setting');
        $this->app_domain = $this->sslcommerz_config?->sslcommerz_test_mode ? 'https://sandbox.sslcommerz.com' : 'https://securepay.sslcommerz.com';

        $this->setStoreId($this->sslcommerz_config?->sslcommerz_store_id);
        $this->setStorePassword($this->sslcommerz_config?->sslcommerz_store_password);
    }
    ...
    protected function validate($merchant_trans_id, $merchant_trans_amount, $merchant_trans_currency, $post_data)
    {
        ...
        if (! empty($merchant_trans_id) && ! empty($merchant_trans_amount)) {
            ...
            $requested_url = ($this->app_domain.$this->config['apiUrl']['order_validate'].'?val_id='.$val_id.'&store_id='.$store_id.'&store_passwd='.$store_passwd.'&v=1&format=json');

            $handle = curl_init();
            curl_setopt($handle, CURLOPT_URL, $requested_url);
            curl_setopt($handle, CURLOPT_RETURNTRANSFER, true);

            if ($this->sslcommerz_config?->sslcommerz_localhost) {
    ...
    public function makePayment(array $requestData, $type = 'checkout', $pattern = 'json')
    {
        ...
        $this->setApiUrl($this->app_domain.$this->config['apiUrl']['make_payment']);
        ...
        $response = $this->callToApi($this->data, $header, $this->sslcommerz_config?->sslcommerz_localhost);
