...
class StaffboardController extends Controller
{
    ...
    public function dashboard(Request $request)
    {
        ...
        // Only shops THIS staff actually created (created_by column)
        ...
        file_put_contents(base_path('debug-8fd336.log'), json_encode([
            'sessionId' => '8fd336',
            'runId' => 'post-fix',
            'hypothesisId' => 'SHOP_COUNT',
            'location' => 'StaffboardController@dashboard',
            'message' => 'staff dashboard shop counts',
            'data' => [
                'staffId' => $staffId,
                'totalShops' => $totalShops,
                'allShops' => Shop::count(),
                'byCreatedBy' => Shop::where('created_by', $staffId)->count(),
                'byStaffIdNullCreatedBy' => Shop::whereNull('created_by')->where('staff_id', $staffId)->count(),
                'totalJobs' => $totalJobs,
