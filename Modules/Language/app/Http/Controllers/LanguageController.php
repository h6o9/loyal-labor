    ...
    public function index(Request $request)
    {
        ...
        if ($request->ajax()) {
            return DataTables::of($query)
                ->addColumn('direction_label', function ($language) {
                    return $language->direction == 'ltr' ? __('Left to right') : __('Right to left');
                })
                ->addColumn('default_toggle', function ($language) {
                    if (!checkAdminHasPermission('language.update')) {
                        return '';
                    }

                    $checked = $language->is_default ? 'checked' : '';

                    return '<a href="javascript:;" onclick="changeStatus(' . $language->id . ', \'is_default\')">'
                        . '<input class="self-default-' . $language->id . ' default-status" id="status_toggle_default_' . $language->id . '" type="checkbox" '
                        . $checked . ' data-toggle="toggle" data-onlabel="' . __('Yes') . '" data-offlabel="' . __('No') . '" data-onstyle="success" data-offstyle="danger">'
                        . '</a>';
                })
                ->addColumn('translations_link', function ($language) {
                    if (!checkAdminHasPermission('language.update')) {
                        return '';
                    }

                    return '<div class="dropdown d-inline"><a class="btn btn-primary" href="'
                        . route('admin.languages.edit-static-languages', $language->code) . '" title="' . __('Edit Language') . '">'
                        . '<i class="fas fa-language"></i></a></div>';
                })
                ->addColumn('status_toggle', function ($language) {
                    if (!checkAdminHasPermission('language.update')) {
                        return '';
                    }
                    ...
                    return '<a href="javascript:;" onclick="changeStatus(' . $language->id . ', \'status\')">'
                        . '<input id="status_toggle_' . $language->id . '" type="checkbox" ' . $checked
                        . ' data-toggle="toggle" data-onlabel="' . __('Active') . '" data-offlabel="' . __('Inactive') . '" data-onstyle="success" data-offstyle="danger">'
                        . '</a>';
                })
                ->addColumn('action', function ($language) {
                    $html = '';

                    if (checkAdminHasPermission('language.edit')) {
                        $html .= '<a class="m-1 text-white btn btn-sm btn-warning" href="' . route('admin.languages.edit', $language->id) . '" title="' . __('Edit') . '"><i class="fa fa-edit"></i></a>';
                    }

                    if (checkAdminHasPermission('language.delete')) {
                        $html .= '<a class="btn btn-danger btn-sm deleteForm" href="javascript:;" data-url="' . route('admin.languages.destroy', $language->id) . '"><i class="fa fa-trash" aria-hidden="true"></i></a>';
                    }

                    return $html;
                })
                ->rawColumns(['default_toggle', 'translations_link', 'status_toggle', 'action'])
                ->make(true);
        }
